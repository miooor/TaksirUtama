...
describe("PBD setup workspace", () => {
  ...
  it("guides incomplete setup in dependency order", () => {
    ...
    expect(resolvePbdSetupView(undefined, setup({ classes: [{ id: "c", name: "1 A", enrolledCount: 20, levelKind: "tahun", levelNumber: 1, teacherName: null, active: true, canDelete: true }] }))).toBe("subjects");
  });

  it("opens assignments once active classes and subjects exist", () => {
    expect(resolvePbdSetupView("unknown", setup({
      classes: [{ id: "c", name: "1 A", enrolledCount: 20, levelKind: "tahun", levelNumber: 1, teacherName: null, active: true, canDelete: true }],
  ...
  it("excludes archived setup records from operational counts", () => {
    const data = setup({
      classes: [
        { id: "c1", name: "1 A", enrolledCount: 20, levelKind: "tahun", levelNumber: 1, teacherName: null, active: true, canDelete: false },
        { id: "c2", name: "1 B", enrolledCount: 20, levelKind: "tahun", levelNumber: 1, teacherName: null, active: false, canDelete: false },
      ],
      subjects: [
