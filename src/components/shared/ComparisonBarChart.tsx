...
export function ComparisonBarChart({
  ...
  return (
    <section className="rounded-xl border border-border-default bg-surface-card p-5 shadow-card">
      ...
      {rows.length === 0 ? <p className="mt-4 text-sm text-text-muted">{emptyMessage}</p> : null}
      <div className="mt-4 space-y-4">
        {rows.map((row) => (
          <div key={row.label}>
            <p className="mb-2 text-sm font-medium text-text-primary">{row.label}</p>
              ...
              {series.map((item) => {
                ...
                return (
                  <div key={item.key} className="grid grid-cols-[5.5rem_1fr_auto] items-center gap-2 text-xs sm:text-sm">
                    ...
                    <span className="tabular-nums text-text-secondary">{row.valueLabels?.[item.key] ?? `${value.toFixed(1)}${suffix}`}</span>
                  </div>
                );
              })}
            </div>
