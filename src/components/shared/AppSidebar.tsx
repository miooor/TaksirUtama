...
function NavLinks({ year, onNavigate }: { year: string; onNavigate?: () => void }) {
  ...
  const groups = [
    ...
    [{ label: "Isi Rumusan TP", href: `/pbd/entry?year=${selectedYear}&semester=${selectedSemester}`, active: pathname === "/pbd/entry" }, { label: "Setup PBD", href: `/pbd/setup?year=${selectedYear}&semester=${selectedSemester}`, active: pathname === "/pbd/setup" }, { label: "Analisis PBD", href: `/pbd/periods/${selectedYear}?semester=${selectedSemester}`, active: pathname.startsWith("/pbd/periods") }],
