...
export function PbdSetupWorkspace({ setup, year, semester, view }: { setup: DatabasePbdSetup; year: string; semester: "1" | "2"; view: PbdSetupView }) {
  ...
  const tabs: Array<{ id: PbdSetupView; label: string; count: number }> = [{ id: "classes", label: "Kelas", count: counts.classes }, { id: "subjects", label: "Subjek", count: counts.subjects }, { id: "assignments", label: "Penetapan", count: counts.assignments }];
  return <div className="mt-6 min-w-0 space-y-5">
    <section className="rounded-lg bg-stone-100 px-4 py-4 sm:px-5"><div className="flex min-w-0 flex-wrap items-center justify-between gap-4"><nav aria-label="Bahagian Setup PBD" className="flex flex-wrap gap-2">{tabs.map((tab) => <Link key={tab.id} href={`/pbd/setup?year=${year}&semester=${semester}&view=${tab.id}`} aria-current={view === tab.id ? "page" : undefined} className={`rounded-md px-3 py-2 text-sm font-medium ${view === tab.id ? "bg-slate-950 text-white" : "text-slate-700 hover:bg-stone-200"}`} style={view === tab.id ? { color: "white" } : undefined}>{tab.label} <span className="ml-1 tabular-nums">{tab.count}</span></Link>)}</nav><p className="text-sm text-slate-600">Semester {semester} · {year}</p></div></section>
