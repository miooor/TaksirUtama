...
export default async function SchoolSetupPage({ searchParams }: { searchParams: Promise<{ year?: string; semester?: string; view?: string }> }) {
  ...
  return <AppShell><PageHeader eyebrow={`Tahun akademik ${year}`} title="Setup Sekolah" description="Urus kelas, murid, subjek dan penetapan yang dikongsi oleh semua pentaksiran." icon={Settings2} actions={<Link href={`/pbd/entry?year=${year}&semester=${semester}`} className="inline-flex items-center justify-center rounded-lg border border-primary-200 bg-primary-50 px-3.5 py-2 text-sm font-semibold text-primary-700 transition-colors hover:bg-primary-100">Isi Rumusan TP</Link>} />{configured && setup && registry ? <SchoolSetupWorkspace setup={setup} registry={registry} year={year} semester={semester} view={view} /> : <section className="mt-6 rounded-xl border border-border-default bg-surface-card p-6 text-sm text-text-muted shadow-card">Pangkalan data belum disambungkan. Tetapkan DATABASE_URL dan jalankan migrasi dahulu.</section>}</AppShell>;
}
