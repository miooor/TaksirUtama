  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={canManagePbd ? <Button href={entryHref} icon={PenLine}>Isi Rumusan TP</Button> : null}
      />
        ...
        <section className="mt-8">
          <div className="flex min-w-0 flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-text-muted">Subjek dengan kelas yang belum selesai, disusun mengikut keutamaan.</p>
            </div>
            {canManagePbd ? (
                ...
                const content = (
                  <>
                    ...
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                      ...
                      {canManagePbd ? <ArrowUpRight className="h-4 w-4 shrink-0 text-text-disabled transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-primary-600" aria-hidden="true" /> : null}
                    </div>
                  </>
                );
                return canManagePbd ? (
                  <Link key={subject.id} href={`/pbd/entry?year=${selectedYear}&semester=${semester}&subjectId=${subject.id}`} className="group flex min-w-0 flex-col gap-3 px-5 py-4 transition-colors hover:bg-surface-inset/60 sm:flex-row sm:items-center sm:justify-between sm:px-6">{content}</Link>
                ) : (
