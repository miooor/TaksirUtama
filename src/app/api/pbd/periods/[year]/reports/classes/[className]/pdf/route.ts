...
export async function GET(_: Request, { params }: { params: Promise<{ year: string; className: string }> }) {
  ...
  return new Response(new Uint8Array(buffer), { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${schoolReportFilename(school, `${analysis.className} - LAPORAN PBD ${period.year}.pdf`)}"` } });
}
