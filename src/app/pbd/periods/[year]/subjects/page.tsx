...
export default async function PbdPeriodSubjectsPage({ params, searchParams }: { params: Promise<{ year: string }>; searchParams: Promise<{ semester?: string }> }) {
  ...
  return (
    <AppShell>
      <h1 className="text-2xl font-semibold">{text(language, { ms: "Subjek / Panitia", en: "Subjects / Panels" })} · Semester {semester} · {period.year}</h1>
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {subjects.map((subject) => (
          <Link key={subject} href={pbdSemesterHref(`${pbdBasePath(period)}/subjects/${encodeURIComponent(subject)}`, semester)} className="rounded-lg border bg-white p-5 font-semibold">
            {subject}
          </Link>
        ))}
      </div>
