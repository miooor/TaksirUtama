-- Aligns pbd_save_class_entries with the subject workflow (005): a finalized
-- class-subject entry keeps its enrolled_count snapshot unless that exact row
-- is reopened. Previously a class-mode draft save for a sibling subject could
-- rewrite the finalized row's denominator when the class enrollment had
-- changed since finalization, leaving TP totals inconsistent with the
-- recorded enrollment.
