CREATE OR REPLACE FUNCTION pbd_save_subject_entries(
  p_school_id text,
  p_period_id text,
  p_subject_id text,
  p_actor_id text,
...
BEGIN
  ...
  PERFORM 1 FROM school_subjects
  WHERE id = p_subject_id AND school_id = p_school_id AND active = true
  FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Subjek tidak ditemui atau telah diarkibkan.'; END IF;

  SELECT count(*) INTO assignment_count
  FROM class_subjects cs
  JOIN school_classes c ON c.id = cs.class_id AND c.active = true
  JOIN database_pbd_periods period ON period.id = p_period_id AND period.academic_year_id = c.academic_year_id
  WHERE cs.school_id = p_school_id AND cs.subject_id = p_subject_id AND cs.active = true;
  SELECT count(*), count(DISTINCT item->>'class_subject_id')
  INTO target_count, distinct_payload_count
  FROM jsonb_array_elements(p_entries) AS item;
  IF assignment_count <> target_count OR target_count <> distinct_payload_count THEN
    RAISE EXCEPTION 'Senarai kelas telah berubah. Muat semula sebelum menyimpan.';
  END IF;
  IF p_finalize_class_subject_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM jsonb_array_elements(p_entries) AS item
    WHERE item->>'class_subject_id' = p_finalize_class_subject_id
  ) THEN
    RAISE EXCEPTION 'Kelas untuk dimuktamadkan tiada dalam senarai semasa.';
  END IF;
  IF p_reopen_class_subject_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM jsonb_array_elements(p_entries) AS item
    WHERE item->>'class_subject_id' = p_reopen_class_subject_id
  ) THEN
    RAISE EXCEPTION 'Kelas untuk dibuka semula tiada dalam senarai semasa.';
  END IF;
  ...
  LOOP
    ...
    WHERE cs.id = payload.class_subject_id AND cs.school_id = p_school_id AND cs.subject_id = p_subject_id AND cs.active = true
    FOR UPDATE OF cs;
    IF NOT FOUND THEN RAISE EXCEPTION 'Kelas untuk subjek ini tidak sah atau telah diarkibkan.'; END IF;
  ...
  IF p_finalize_class_subject_id IS NOT NULL THEN
    ...
    JOIN class_subjects cs ON cs.id = p.class_subject_id AND cs.school_id = p_school_id AND cs.subject_id = p_subject_id AND cs.active = true
    JOIN school_classes c ON c.id = cs.class_id AND c.active = true
    JOIN database_pbd_periods period ON period.id = p_period_id AND period.academic_year_id = c.academic_year_id
  ...
  LOOP
    ...
    next_status := CASE
      ...
      ELSE 'draft'
    END;
    next_enrolled_count := CASE
      WHEN current_entry.id IS NOT NULL AND current_entry.status = 'final'
        AND payload.class_subject_id <> p_reopen_class_subject_id
      THEN current_entry.enrolled_count
      ELSE current_entry.class_enrolled_count
    END;
    changed_row := current_entry.id IS NULL
      OR current_entry.enrolled_count IS DISTINCT FROM next_enrolled_count
    ...
    IF changed_row THEN
      ...
      IF current_entry.id IS NULL THEN
        ...
        VALUES (created_entry_id, p_school_id, p_period_id, payload.class_subject_id, next_enrolled_count, payload.tp1, payload.tp2, payload.tp3, payload.tp4, payload.tp5, payload.tp6, payload.not_assessed, next_status, 1, p_actor_id);
        ...
        VALUES (gen_random_uuid()::text, p_school_id, created_entry_id, p_actor_id, 'create', '{}'::jsonb,
          jsonb_build_object('enrolledCount', next_enrolled_count, 'tp1', payload.tp1, 'tp2', payload.tp2, 'tp3', payload.tp3, 'tp4', payload.tp4, 'tp5', payload.tp5, 'tp6', payload.tp6, 'notAssessed', payload.not_assessed, 'status', next_status));
      ELSE
  ...
  END LOOP;
  RETURN QUERY
    SELECT cs.id, e.id, COALESCE(e.revision, 0), COALESCE(e.status, 'draft'), COALESCE(e.enrolled_count, c.enrolled_count),
